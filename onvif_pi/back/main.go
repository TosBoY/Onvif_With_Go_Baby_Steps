...
func getResolutions(w http.ResponseWriter, r *http.Request) {
	...
	// Keep GovLengthRange and encodingIntervals to preserve the GOP length range values from pi
	// Log the values if they exist, for informational purposes
	if govRange, ok := options["GovLengthRange"]; ok {
		log.Printf("getResolutions: Found GovLengthRange from Pi: %v", govRange)
	}

	if intervals, ok := options["encodingIntervals"]; ok {
		log.Printf("getResolutions: Found encodingIntervals from Pi: %v", intervals)
	}
